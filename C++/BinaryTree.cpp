...
NodeType * BiTree::creat()                      //递归建立二叉树算
{
    NodeType *p;
    ElemType x;
    cout<<"\n 输入数据：";
...
int BiTree::height(NodeType *p)                //求二叉树高度递归
{
    /*int tHeight = 0;
    if(p!= NULL){
        int leftHeight = height(p->lch);
        int rightHeight = height(p->rch);
        int tHeight = leftHeight>rightHeight?leftHeight +1 :rightHeight + 1;
    }
    cout<<tHeight;
    return tHeight;*/
    //if (p = NULL) return 0;
    ...
    */
    if( p !=null){
        int tleft = height(p->lch);
        int tright = height(p->rch);
