...
void TFT_Init(void)
{
	...
	TFT_CLEAR(WHITE);  
//	POINT_COLOR=RED;            //更改字体颜色
}

void TFT_Desk(void)
{
		TFT_DrawLine(50,120,50,245);    //画框
		TFT_DrawLine(50,120,410,120);
		TFT_DrawLine(410,120,410,245);
		TFT_DrawLine(50,245,410,245);
	
		TFT_ShowChinese(7,9,1);//显示 卡
		TFT_ShowChinese(8,9,2);//显示 号
		TFT_ShowChinese(9,9,3);//显示 ：
...
void TFT_Show_ID(void)
{ 
	u8 i;
  MFRC_AppReadId(ID);				//读取卡号
//--------------------------------------------------方法一-------------------------
							//将4位的数值转换为8位的字符
//							if((ID[0]&0x0f)>9)
...
//--------------------------------------------------方法二----------------------------
	for(i=0;i<4;i++)
	{
		sprintf(&id[i*2],"%x",ID[i]);	
		TFT_ShowString(158,144,id);	//144*16,纵坐标的位置，158横坐标的位置
  }		
}

void Clear_TFT(void)
{
		TFT_ShowString(158,144,"                   ");//清除X=157后面的显示内容
}
